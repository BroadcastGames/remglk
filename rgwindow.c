...
{
    /* Do an immediate printf here so we can try to match up with styleset.
     It's all about the timing of when the window is created and the current values. */
    /* include extra newline after stanza */
    printf("{ \"type\":\"windowRef\", \"note0\":\"stylesetcolor\", \"BackColor_value0\":%d, \"BackColor_value1\":%d, \"BackColor_value2\":%d, \"BackColor_value3\":%d, \"refpoint\":\"glk_window_open\", \"wintype\":%d, \"rock\":%d }\n\n",
        stylehint_values[style_Normal][wintype][stylehint_BackColor], stylehint_values[style_Normal][wintype_AllTypes][stylehint_BackColor], stylehint_values[style_Normal][wintype_TextBuffer][stylehint_BackColor], stylehint_values[style_Normal][wintype_TextGrid][stylehint_BackColor],
        wintype, rock);
    /* IDEA: This is the point to capture all of the stylehint values and emit them? */
